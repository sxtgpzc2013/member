 ...
 */
	class BonusAction extends CommonAction {
	    ...
	    public function flow()
	    {
			...
			$where = "targetuserid = {$_SESSION['Rongzi']['user']['uid']}";

			if($start && $stop){
				$where = "createtime >= ".strtotime($start)." AND createtime <= ".(strtotime($stop)+24*60*60) . " AND (targetuserid = {$_SESSION['Rongzi']['user']['uid']} )";
			}
