...
<div class="con-list">
	...
	<!-- 	<a href="__APP__/Products/add" class="preview">新增</a> -->
		<a href="__APP__/Products/add" class="preview">新增</a>
		<ul class="tab_menu">
			<li class="current">
				<a href="__APP__/Products/index/menu_id/<?php echo ($result["menu_id"]); ?>">查看信息</a>
			</li>
			
		</ul>
	</div>

	<?php if(!empty($result['products'])): ?><table class="content-table">
			<thead>
				<tr> 
					<th>序号</th>
					<th>名称</th>
			...
			<tbody>
				<?php if(is_array($result['products'])): foreach($result['products'] as $key=>$info): ?><tr>
						<td><?php echo ($info["id"]); ?></td>
						<td><?php echo ($info["name"]); ?></td>
						<td>售价￥<?php echo ($info["price"]); ?></td>
						<td><?php if($info['is_recommend'] == 'yes'): ?>推荐<?php else: ?>不推荐<?php endif; ?></td>
						<td><img src="__ROOT__/Uploads/images/products/l_<?php echo ($info["logo"]); ?>" alt="<?php echo ($info["name"]); ?>" style="width: 50px; height: 50px;"></td>
						<td><?php echo (date("Y-m-d H:i:s",$info['created_at'])); ?></td>
						<td>
							<a href="__APP__/Products/edit/id/<?php echo ($info['id']); ?>">编辑</a>

							<a href="__APP__/Products/delete/id/<?php echo ($info['id']); ?>" onclick="return confirm('确定删除吗？')">删除</a>
						</td>
